...
def create_experiment_name(model_name, main_config, model_config):
    experiment_name = "{}_{}".format(
        model_name, main_config["PARAMS"]["embedding_size"]
    )
...
def train(
    ...
    (
        ...
        train_mini_sen2,
        train_mini_labels,
    ) = dataset_helper.pick_train_mini_batch()
    ...
    with tf.Session(config=config) as session:
        ...
        for epoch in tqdm(range(main_cfg.num_epochs), desc="Epochs"):
            ...
            for batch in tqdm_iter:
                ...
                loss, _ = session.run(
                    [model.loss, model.opt], feed_dict=feed_dict_train
                )
