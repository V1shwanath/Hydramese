...
class MultiheadSiameseNetGuiDemo:
    def __init__(self, master):
        ...
        self.resultLabel = Label(
            self.frame, text="Result", font="Helvetica {}".format(GUI_FONT_SIZE)
        )
        self.first_sentence_label = Label(
            self.frame, text="Sentence 1", font="Helvetica {}".format(GUI_FONT_SIZE)
        )
        self.second_sentence_label = Label(
            self.frame, text="Sentence 2", font="Helvetica {}".format(GUI_FONT_SIZE)
        )
        ...
        self.model_type = OptionMenu(
            master, variable, *model_dirs, command=self.load_model
        )
    ...
    def load_model(self, model_name):
        if "multihead" in model_name:
            self.visualize_attentions_checkbox.grid(
                row=2, column=0, sticky=W + E, ipady=1
            )
        else:
        ...
        self.model = model(
            self.max_doc_len, self.vocabulary_size, self.main_config, model_config
        )
        saver = tf.train.Saver()
        last_checkpoint = tf.train.latest_checkpoint(
            "{}/{}".format(self.model_dir, model_name)
        )
