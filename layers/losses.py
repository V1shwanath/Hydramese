...
def contrastive(predictions, labels):
    ...
    contrastive_loss_plus = (1.0 - tf.to_float(labels)) * _contrastive_minus(
        predictions
    )
...
def cross_entropy(predictions, labels):
    ...
    return tf.reduce_mean(
        -tf.reduce_sum(labels * tf.log(predictions + _EPSILON), axis=1)
    )
...
def contrastive_lecun(x1, x2, labels, margin=0.2, distance=manhattan_distance):
    ...
    c_loss = labels * 0.5 * tf.square(distance(x1, x2)) + (
        1 - labels
    ) * 0.5 * tf.square(tf.maximum(0.0, margin - distance(x1, x2)))
